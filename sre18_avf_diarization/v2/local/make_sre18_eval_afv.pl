...
################### change for eval
open(META, "<$db_base/docs/sre18_dev_enrollment_afv.tsv") or die "cannot open wav list";
%utt2fixedutt = ();
while (<META>) {
  $line = $_;
  @toks = split(" ", $line);
...
################### change for eval
open(KEY, "<$db_base/docs/sre18_dev_trial_key_afv.tsv") || die "Could not open trials file $db_base/docs/sre16_eval_trial_key.tsv.  It might be located somewhere else in your distribution.";
open(SEG_KEY, "<$db_base/docs/sre18_dev_segment_key_afv.tsv") || die "Could not open trials file $db_base/docs/sre16_eval_segment_key.tsv.  It might be located somewhere else in your distribution.";
