...
def compute_min_dcf_idx(fnr, fpr, p_target, c_miss=1, c_fa=1):
    """ computes normalized minimum detection cost function (DCF) given
        the costs for false accepts and false rejects as well as a priori
        probability for target speakers
    """
...
def plot_det_curve(fnr, fpr):
    ...
    plt.xlim(norm.ppf([0.00001, 0.8]))
    plt.ylim(norm.ppf([0.00001, 0.8]))
    ...
    plt.annotate("EER = {:.2f}%".format(eer * 100),
                 ...
                 xycoords='data',
                 xytext=(norm.ppf(eer - 0.01), norm.ppf(eer - 0.09)),
                 textcoords='data',
                 arrowprops=arrowops,
                 size=12, va='center', ha='center',
                 bbox=dict(boxstyle="round4", fc="w"),)

    # c_det, c_idx = compute_min_dcf_idx(fnr, fpr, 0.01)
    # plt.annotate("C_MIN = {:.2f}".format(c_det),
    #              xy=(norm.ppf(fpr[c_idx]), norm.ppf(fnr[c_idx])),
    #              xycoords='data',
    #              xytext=(norm.ppf(fpr[c_idx] + 0.05), norm.ppf(fnr[c_idx] + 0.05)),
    #              textcoords='data',
    #              arrowprops=arrowops,
    #              size=12, va='center', ha='center',
    #              bbox=dict(boxstyle="round4", fc="w"),)
