...
from sre_scorer import plot_det_curve
...
def plot_det(scores, partition_masks, tar_nontar_labs, ptar):
    scores, labels, weights = compute_partition_weights(scores,
                                                        partition_masks,
                                                        tar_nontar_labs)
    fnr, fpr = sc.compute_pmiss_pfa_rbst(scores, labels, weights)
    plot_det_curve(fnr, fpr)
    c_det, c_idx = sc.compute_min_dcf_idx(fnr, fpr, ptar[0])
    from scipy.stats import norm
    import matplotlib.pyplot as plt
    arrowops = dict(arrowstyle="-|>", connectionstyle="arc3, rad=+0.2", fc="w")
    plt.annotate("C_MIN_a = {:.2f}".format(c_det),
                 xy=(norm.ppf(fpr[c_idx]), norm.ppf(fnr[c_idx])),
                 xycoords='data',
                 xytext=(norm.ppf(fpr[c_idx] + 0.00), norm.ppf(fnr[c_idx] - 0.35)),
                 textcoords='data',
                 arrowprops=arrowops,
                 size=12, va='center', ha='center',
                 bbox=dict(boxstyle="round4", fc="w"),)
    if len(ptar) > 1:
        c_det, c_idx = sc.compute_min_dcf_idx(fnr, fpr, ptar[1])
        plt.annotate("C_MIN_b = {:.2f}".format(c_det),
                 xy=(norm.ppf(fpr[c_idx]), norm.ppf(fnr[c_idx])),
                 xycoords='data',
                 xytext=(norm.ppf(fpr[c_idx] - 0.00), norm.ppf(fnr[c_idx] - 0.25)),
                 textcoords='data',
                 arrowprops=arrowops,
                 size=12, va='center', ha='center',
                 bbox=dict(boxstyle="round4", fc="w"),)
