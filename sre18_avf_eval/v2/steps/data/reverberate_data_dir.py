...
def GenerateReverberationOpts(room_dict,  # the room dictionary, please refer to MakeRoomDict() for the format
    ...
    if len(noise_addition_descriptor['noise_io']) > 0:
        ...
        reverberate_opts += "--start-times='{0}' ".format(','.join(list(map(lambda x:str(x), noise_addition_descriptor['start_times']))))
        reverberate_opts += "--snrs='{0}' ".format(','.join(list(map(lambda x:str(x), noise_addition_descriptor['snrs']))))
...
def AddPrefixToFields(input_file, output_file, num_replicas, include_original, prefix, field = [0]):
    list_list = list(map(lambda x: x.strip(), open(input_file)))
    # list = map(lambda x: x.strip(), open(input_file)) # nave
...
def CreateReverberatedCopy(input_dir,
    ...
    foreground_snr_array = list(map(lambda x: float(x), foreground_snr_string.split(':')))
    background_snr_array = list(map(lambda x: float(x), background_snr_string.split(':')))
...
def ParseRirList(rir_set_para_array, smoothing_weight, sampling_rate = None):
    ...
    for rir_set in set_list:
        current_rir_list = list(map(lambda x: rir_parser.parse_args(shlex.split(x.strip())),open(rir_set.filename)))
...
def ParseNoiseList(noise_set_para_array, smoothing_weight, sampling_rate = None):
    ...
    for noise_set in set_list:
        current_noise_list = list(map(lambda x: noise_parser.parse_args(shlex.split(x.strip())),open(noise_set.filename)))
