...
'
  ...
  local/make_sre18_eval_afv_eval.pl $data_location/LDC2018E512 data
...
if [ $stage -le 7 ]; then
  # The SRE16 test data
  sid/nnet3/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 64G" --chunk-size $chunck --nj 30 \
    $nnet_dir data/sre16_eval_test \
    exp/xvectors_sre16_eval_test -1

  # The SRE16 enroll data
  sid/nnet3/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 64G" --chunk-size $chunck --nj 30 \
    $nnet_dir data/sre16_eval_enroll \
    exp/xvectors_sre16_eval_enroll -1
fi
